...
public final class DigsSyncManager
{
    ...
    public static void onClientTick(long now)
    {
        ...
        if (detection.chosenTotal() > 0L)
        {
            ...
            String resolvedSourceName = ScoreboardSourceResolver.displayName(
                    worldInfo != null ? worldInfo.displayName() : "",
                    worldInfo
            );
            debug("bootstrap-source-selected source={} chosenTotal={} sidebarTotal={} tabTotal={}",
                    detection.chosenSource(),
                    detection.chosenTotal(),
                    detection.sidebarTotal(),
                    detection.tabTotal());
            MiningStats.bootstrapSourceTotalFromScoreboard(detection.chosenTotal(), resolvedSourceName, now);
        }
        ...
        {
            latestModel = parsed;
            debug("scoreboard-player-total-detected username={} total={} sourceName={}", parsed.username(), parsed.totalDigs(), parsed.server());
            MiningStats.bootstrapSourceTotalFromScoreboard(parsed.totalDigs(), parsed.server(), now);
            MiningStats.applyAeternumTotalMined(parsed.totalDigs(), now);
            debug("Parsed player total digs username={} total={} objective={}", parsed.username(), parsed.totalDigs(), parsed.objectiveTitle());
